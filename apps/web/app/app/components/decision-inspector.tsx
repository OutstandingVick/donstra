...
export function DecisionInspector({ scenario }: { scenario: Scenario }) {
  ...
  return (
    <section className="mb-8 mt-12" aria-labelledby="inspector-title">
      <div className="mb-5">
        <p className="m-0 text-xs font-semibold uppercase tracking-[0.14em] text-white/35">Committed testimony</p>
        ...
        <div className="overflow-x-auto border-b border-white/10" role="tablist" aria-label="Decision evidence">
          <div className="flex min-w-max px-2">
            {tabs.map((tab, index) => {
              ...
              return <button id={`inspector-tab-${index}`} key={tab} type="button" role="tab" aria-selected={active} aria-controls="inspector-panel" tabIndex={active ? 0 : -1} onKeyDown={(event) => moveTab(event, index)} onClick={() => setActiveTab(tab)} className={`relative min-h-12 px-4 text-sm font-medium focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-[-2px] focus-visible:outline-[#4BED86] ${active ? "text-white" : "text-white/40 hover:text-white/70"}`}>{tab}{active && <span className="absolute inset-x-3 bottom-0 h-px bg-[#16DB65]" aria-hidden="true" />}</button>;
            })}
          </div>
        </div>
        <div id="inspector-panel" role="tabpanel" aria-labelledby={`inspector-tab-${tabs.indexOf(activeTab)}`} className="grid lg:grid-cols-[minmax(0,1fr)_18rem]">
          <div className="p-5 sm:p-6">
            ...
            <dl className="m-0 grid gap-px overflow-hidden rounded-lg border border-white/10 bg-white/10 sm:grid-cols-2">
              {pane.fields.map((field) => <div key={field.label} className="min-w-0 bg-[#0D160B] p-4"><dt className="text-xs text-white/35">{field.label}</dt><dd className={`m-0 mt-2 break-words text-sm leading-6 ${field.value.startsWith("0x") ? "font-mono text-xs" : ""} ${field.emphasis === "danger" ? "text-[#FF9698]" : field.emphasis === "verified" ? "text-[#80F4A9]" : "text-white/75"}`}>{field.value}</dd></div>)}
            </dl>
            <details className="group mt-5 rounded-lg border border-white/10 bg-white/[0.02]">
              <summary className="flex min-h-11 cursor-pointer list-none items-center justify-between px-4 text-sm font-medium text-white/60 focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-[-2px] focus-visible:outline-[#4BED86]">View raw representation<ChevronDown size={16} className="transition-transform duration-150 group-open:rotate-180" aria-hidden="true" /></summary>
              <pre className="m-0 overflow-x-auto border-t border-white/10 p-4 font-mono text-xs leading-6 text-white/45"><code>{pane.raw}</code></pre>
            </details>
          </div>
          <aside className="border-t border-white/10 bg-white/[0.02] p-5 lg:border-s lg:border-t-0" aria-label="Binding status">
            <p className="m-0 text-xs font-semibold uppercase tracking-[0.12em] text-white/35">Verification</p>
