  ...
  return (
    <section className="mt-12" aria-labelledby="comparison-title">
      <div className="mb-5 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="m-0 text-xs font-semibold uppercase tracking-[0.14em] text-white/50">Same incident, different proof</p>
          <h2 id="comparison-title" className="mb-0 mt-2 text-2xl font-semibold tracking-[-0.025em] text-white">Execution traces</h2>
        </div>
        <p className="m-0 max-w-lg text-sm leading-6 text-white/60">The traces diverge at the point where an explanation must become independently verifiable.</p>
      </div>
        ...
        <div className="grid md:grid-cols-2">
          <ol className="m-0 list-none p-0 md:border-e md:border-white/10">
            {scenario.withoutDonstra.map((step, index) => (
              <li key={step.title} className="grid grid-cols-[2rem_1fr] gap-3 border-b border-white/[0.07] p-4 last:border-b-0 sm:p-5">
                <span className={`mt-0.5 grid size-7 place-items-center rounded-md border ${step.state === "unverifiable" ? "border-[#A94C4E]/40 bg-[#5C1A1B]/20 text-[#FF9698]" : "border-white/10 bg-white/[0.035] text-white/60"}`}>
                  {step.state === "unverifiable" ? <FileWarning size={14} aria-hidden="true" /> : <Check size={14} aria-hidden="true" />}
                </span>
                <div><p className="m-0 text-sm font-semibold text-white/80">{index + 1}. {step.title}</p><p className="mb-0 mt-1 text-sm leading-6 text-white/55">{step.detail}</p></div>
              </li>
            ))}
          </ol>
          <ol className="m-0 list-none p-0">
            {scenario.withDonstra.map((step, index) => {
              ...
              return (
                <li key={step.title} className="grid grid-cols-[2rem_1fr] gap-3 border-b border-white/[0.07] p-4 last:border-b-0 sm:p-5">
                  <span className={`mt-0.5 grid size-7 place-items-center rounded-md border ${complete ? "border-[#16DB65]/30 bg-[#16DB65]/10 text-[#4BED86]" : "border-white/10 text-white/45"}`}>
                    {complete ? <Check size={14} aria-hidden="true" /> : <Circle size={10} aria-hidden="true" />}
                  </span>
                  <div className="min-w-0"><p className={`m-0 text-sm font-semibold ${complete ? "text-white/90" : "text-white/50"}`}>{index + 1}. {step.title}</p><p className="mb-0 mt-1 text-sm leading-6 text-white/55">{step.detail}</p>{step.proof && <p className="mb-0 mt-2 overflow-hidden text-ellipsis whitespace-nowrap font-mono text-xs text-white/50" title={step.proof}>{step.proof}</p>}</div>
                </li>
              );
            })}
          </ol>
