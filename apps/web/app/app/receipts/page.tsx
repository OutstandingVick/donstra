    ...
    {!loading && !error && receipts.length > 0 && <div className="mt-8 overflow-hidden rounded-xl border border-white/10 bg-[#0A1109]">
      ...
      <ul className="m-0 list-none p-0">
        {receipts.map((receipt) => <li key={receipt.id} className="border-b border-white/[0.07] last:border-b-0">
          <Link href={`/app/receipts/detail?id=${encodeURIComponent(receipt.id)}`} className="grid min-h-20 items-center gap-3 px-5 py-4 hover:bg-white/[0.025] focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-[-2px] focus-visible:outline-[#4BED86] lg:grid-cols-[minmax(13rem,1.2fr)_0.8fr_0.8fr_0.8fr_2rem] lg:gap-4">
